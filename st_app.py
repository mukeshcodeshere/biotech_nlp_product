...
file_path = "sec_data/sec_data_all_tickers.csv"
...
def load_data(file_path):
    try:
        # Ensure the file was generated by the download process
        if not os.path.exists(file_path):
            raise FileNotFoundError(f"The file '{file_path}' was not found. Please run the download process first.")

        # Check if the file is empty
        if os.path.getsize(file_path) == 0:
            raise ValueError(f"The file '{file_path}' is empty. Please ensure the data was downloaded correctly.")
        ...
        if df.empty:
            raise ValueError(f"The file '{file_path}' does not contain valid data.")
        ...
        st.error(f"The file '{file_path}' is empty or has no valid CSV data.")
        return None
    except pd.errors.ParserError:
        st.error(f"The file '{file_path}' could not be parsed as CSV. It may be corrupted or incorrectly formatted.")
        return None
    except Exception as e:
        st.error(f"An unexpected error occurred: {e}")
        return None

# Step 3: after Step 2, attempt to load the data (this is now conditional based on Step 2's success)
if os.path.exists(file_path):  # Only try to load data if Step 2 didn't fail
    df_all = load_data(file_path)
    if df_all is not None:
        st.write(df_all)
else:
    st.warning("Please run the download process first to generate the data file.")
